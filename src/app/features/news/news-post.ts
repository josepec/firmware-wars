  ...
  template: `
    <div class="min-h-screen p-6 md:p-10 max-w-3xl mx-auto">
      ...
      <a routerLink="/noticias"
         class="inline-flex items-center min-h-11 text-xs tracking-[0.15em] text-green-400/80 hover:text-green-300 uppercase">
        ← PATCH_NOTES.LOG
      </a>

      @if (loading()) {
        <div class="text-xs text-green-400/85 tracking-wider animate-pulse py-10">
          > CARGANDO ENTRADA...
        </div>
      } @else if (!post()) {
        <div class="mt-8 border border-red-500/30 bg-red-500/5 px-5 py-8 text-center
                    text-xs tracking-[0.15em] text-red-400/90">
        ...
        <article class="mt-6">
          ...
          <div class="text-xs tracking-[0.15em] text-green-400/80 mb-2">
